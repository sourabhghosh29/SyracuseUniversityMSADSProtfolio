...
dim(df_nps_complete)
df_nps_complete[,-25]
random_forest <- randomForest(df_nps_complete$type_passenger ~., data = df_nps_complete[,c(3,4,6:15,21:23,25,26,32)],importance = TRUE, proximity = TRUE,ntree=500)
...
install.packages("randomForest")
library(randomForest)
split <- sample.split(df_nps_complete$Likelihood.to.recommend, SplitRatio = 0.75)
dresstrain <- subset(df_nps_complete, split == TRUE,na.rm=TRUE)
dresstest <- subset(df_nps_complete, split == FALSE,na.rm=TRUE)
model1 <- randomForest(dresstrain$Likelihood.to.recommend ~ . , ntree = 500, data = dresstrain, importance = TRUE)
model1
predTrain <- predict(model1, dresstrain, type = "class")
# Checking classification accuracy
table(predTrain, dresstrain$Likelihood.to.recommed) 

#Random Forest 2
random_forest <- randomForest(dresstrain$type_passenger ~., data = dresstrain[,c(3,4,6:15,21:23,25,26,32)],importance = TRUE,t,importance = TRUE, proximity = TRUE,ntree=500)
summary(random_forest)
print(random_forest)
# importance
round(importance(random_forest),2)
...
y_rf <- dresstrain$Likelihood.to.recommend

x_rf<-data.frame(dresstrain%>% select(Partner.Name,Age,Gender,Airline.Status,Price.Sensitivity,Loyalty,Type.of.Travel,Total.Freq.Flyer.Accts
                                      ,Class,Flights.Per.Year,Departure.Delay.in.Minutes,Arrival.Delay.in.Minutes,Flight.time.in.minutes
                                      ,Flight.Distance,Flight.cancelled))
...
set.seed(seed)
mtry <- sqrt(ncol(x_rf))
dresstrain$Likelihood.to.recommend <- as.factor(dresstrain$Likelihood.to.recommend)
rf_random <- caret::train(Likelihood.to.recommend ~., data=dresstrain, method="rf", metric=metric, tuneLength=15, trControl=control)
